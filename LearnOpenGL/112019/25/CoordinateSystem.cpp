...
int main () 
{
    ...
    while(true) 
    {
        ...
        float mouseX = (static_cast<float>(x)) / 800.0f;
        float mouseY = (static_cast<float>(y)) / 600.0f;  
        float time = SDL_GetTicks()/ 1024.0f;
        GLint uniModel = glGetUniformLocation(shaderProgram, "model");
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::rotate(
            model,
            std::hypot(mouseX, mouseY) * glm::radians(55.0f),
            glm::vec3(-mouseY, -mouseX, 0.0)
        );
        glUniformMatrix4fv(uniModel, 1, GL_FALSE, glm::value_ptr(model));


        glm::mat4 proj = glm::perspective(
            glm::radians(10.0f * -sin(time) + 60.0f ),
            800.0f/600.0f,
            1.0f,
            10.0f
        );    
        ...
        glm::mat4 view = glm::lookAt(
        glm::vec3(sin(time) * 1.2f, 1.2f, cos(time) * 1.2f),
        glm::vec3(0.0f, 0.0f, 0.0f),
        glm::vec3(0.0f, 0.0f, 1.0f)
        );
        glUniformMatrix4fv(uniView, 1, GL_FALSE, glm::value_ptr(view));
